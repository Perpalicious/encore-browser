...
from .evaluate import (EVIDENCE_KINDS, REQUIRED_CRITICAL, evaluate,
...
def progress(session: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"total": len(session["items"]), "reviewed": len(reviewed),
            "by_bucket": {name: {"positive": positive[name], "negative": negative[name]}
...
HTML = r"""<!doctype html><meta charset="utf-8"><title>Gold review</title>
<style>body{font:16px system-ui;max-width:1100px;margin:auto;padding:1rem}button,input,textarea,select{font:inherit}textarea{width:100%;height:5rem}.grid{display:grid;grid-template-columns:1fr 1fr;gap:1rem}.muted{color:#666}.images img{max-height:220px;max-width:30%}.prediction{white-space:pre-wrap;background:#eee;padding:1rem}label{display:block;margin:.5rem 0}</style>
<h1>Human gold review</h1><p id="progress"></p><div id="app"></div>
<script>
let data,index=0,revealed=false;
const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
async function load(){data=await (await fetch('/api/session'+(revealed?'?reveal=1':''))).json(); render()}
function list(v){return (v||[]).join('\n')}
function render(){let x=data.items[index],s=x.source,r=x.review,p=data.progress;
 document.querySelector('#progress').textContent=`${p.reviewed}/${p.total} reviewed — item ${index+1}/${p.total}`;
 let imgs=[s.image_url,s.thumb_url,...(s.additional_images||[])].filter(Boolean);
 document.querySelector('#app').innerHTML=`<details><summary>Bucket and sampling coverage</summary><pre>${esc(JSON.stringify({reviewed:p.by_bucket,sampling:p.sampling.coverage},null,2))}</pre></details>
 <details><summary>Rare-bucket exemptions</summary><p>Explicit reviewed justification only.</p><textarea id="exemptions">${esc(JSON.stringify(data.rare_bucket_exemptions,null,2))}</textarea><button onclick="saveExemptions()">Save exemptions</button></details>
 <p><button onclick="move(-1)">← Previous</button> <button onclick="move(1)">Next →</button> <button onclick="reveal()">${revealed?'Hide':'Reveal'} predictions</button></p>
 <h2>${esc(s.lot_number)} — ${esc(s.title)}</h2><p>${esc(s.condition)} · ${esc(s.hibid_category_path||s.category||(s.category_path||[]).join(' › '))}</p>
 <p>${esc(s.description_raw||s.description||'')}</p><p class="muted">${revealed?'Selected because: '+x.selection_keys.map(esc).join(', '):x.selection_notice}</p>
 <div class="images">${imgs.map(u=>`<img loading="lazy" src="${esc(u)}">`).join('')}</div>
 ${revealed?`<pre class="prediction">${esc(JSON.stringify(x.prediction,null,2))}</pre>`:''}
 <div class="grid"><div><label>Expected buckets (one per line)<textarea id="expected">${esc(list(r.expected_buckets))}</textarea></label>
 <label>Forbidden buckets (one per line)<textarea id="forbidden">${esc(list(r.forbidden_buckets))}</textarea></label>
 <label>Expected subtype <input id="subtype" value="${esc(r.expected_subtype||'')}"></label></div>
 <div><label>Evidence kinds (one per line)<textarea id="evidence">${esc(list(r.expected_evidence_kinds))}</textarea></label>
 <label>Critical assertions (one per line)<textarea id="critical">${esc(list(r.critical_assertions))}</textarea></label>
 <label>Expected personal <select id="personal"><option value="">Choose…</option><option value="true" ${r.expected_personal===true?'selected':''}>Yes</option><option value="false" ${r.expected_personal===false?'selected':''}>No</option></select></label></div></div>
 <label>Human rationale<textarea id="rationale">${esc(r.rationale)}</textarea></label><label><input type="checkbox" id="reviewed" ${r.reviewed?'checked':''}> Reviewed and complete</label>
 <button onclick="save()">Save (Ctrl/⌘+S)</button><p id="message"></p>`}
function lines(id){return document.querySelector(id).value.split('\n').map(x=>x.trim()).filter(Boolean)}
async function save(){let personal=document.querySelector('#personal').value,x=data.items[index]; let review={expected_buckets:lines('#expected'),forbidden_buckets:lines('#forbidden'),expected_subtype:document.querySelector('#subtype').value.trim()||null,expected_evidence_kinds:lines('#evidence'),expected_personal:personal===''?null:personal==='true',critical_assertions:lines('#critical'),rationale:document.querySelector('#rationale').value,reviewed:document.querySelector('#reviewed').checked};
 let res=await fetch('/api/item/'+encodeURIComponent(x.id),{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':data.csrf_token},body:JSON.stringify({review,expected_revision:x.revision})}); let body=await res.json(); document.querySelector('#message').textContent=res.ok?'Saved':body.error; if(res.ok) await load()}
async function saveExemptions(){let value;try{value=JSON.parse(document.querySelector('#exemptions').value)}catch(e){alert('Invalid exemption JSON');return}let res=await fetch('/api/exemptions',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':data.csrf_token},body:JSON.stringify({rare_bucket_exemptions:value,expected_revision:data.revision})});let body=await res.json();if(!res.ok)alert(body.error);else await load()}
function move(n){index=Math.max(0,Math.min(data.items.length-1,index+n));revealed=false;load()} function reveal(){revealed=!revealed;load()}
document.addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key==='s'){e.preventDefault();save()} if(e.altKey&&e.key==='ArrowRight')move(1);if(e.altKey&&e.key==='ArrowLeft')move(-1)});load();
</script>"""
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if parsed.path == "/api/session":
                session = store.read()
                reveal = parse_qs(parsed.query).get("reveal") == ["1"]
                items = []
                for item in session["items"]:
                    public = {key: item[key] for key in ("id", "source", "review", "revision")}
                    public["selection_notice"] = "Selection context is hidden until predictions are revealed."
                    if reveal:
                        public["prediction"] = item["prediction"]
                        public["selection_keys"] = item["selection_keys"]
                    items.append(public)
                self._json(200, {"items": items, "progress": progress(session),
                                 "revision": session["revision"],
                                 "csrf_token": session["csrf_token"],
                                 "rare_bucket_exemptions": session.get("rare_bucket_exemptions", {})}); return
        ...
        def do_POST(self) -> None:  # noqa: N802
            ...
            try:
                ...
                item = store.update(parts[3], body["review"], actor, body["expected_revision"])
